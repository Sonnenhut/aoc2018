...
export const HORIZONTAL_EXAMPLE_2 = [`-------`.split('')];
...
export default async function main() {
    HORIZONTAL_EXAMPLE_0.inflated().deflated().toBe(HORIZONTAL_EXAMPLE_0);
    ...
    fileAsText('day13/example_a_ticks.txt').then(input => {
...
export function tick(state) {
    ...
    state.forEach((y,yIdx) => {
        y.forEach((x,xIdx) => set(state[yIdx][xIdx], yIdx, xIdx, res))
    });

    let alreadyMoved = [];
    for(let y = 0; y < res.length; y++) {
        for(let x = 0; x < res[y].length; x++) {
            if(!alreadyMoved.includes(res[y][x])) {
                ...
                if(mod.noop) {
                    ...
                    // dont look at this cell anymore, has already moved
                    alreadyMoved.push(mod.new)
                }
            }
        }
    }
...
function modify(y, x, state) {
    ...
    let toOverwrite = state[res.y][res.x];
    ...
    if(toOverwrite.isIntersection() && cell.isMoving()) {
        ...
        // was a crash.
        res.new = new Cell(toOverwrite.oldSelf, 0, 'X'); // BOOM CRASH!
